...
def normalize_signals(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert col names to lowercase, rename date->timestamp, parse timestamps, 
    ...
    if 'timestamp' in df.columns:
        ...
        raise KeyError("[signal_loader] Missing 'timestamp' or 'date' in signals")
...
def deduplicate_signals(existing_df: pd.DataFrame, new_df: pd.DataFrame) -> pd.DataFrame:
    """
    Combine existing + new, removing duplicates by symbol,signal,timestamp
    """
    combined = pd.concat([existing_df, new_df], ignore_index=True)
    combined.drop_duplicates(subset=['symbol','signal','timestamp'], inplace=True)
